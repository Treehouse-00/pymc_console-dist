...
## [0.9.394] - 2026-09-17
### Added
- **Set a companion's Path Hash Size from the console.** Each companion the repeater hosts stamps its outgoing paths with 1-, 2- or 3-byte route hashes, and until now only a phone app could change that. Open the companion's Edit dialog (the pencil in Messages → CONFIG) or the new Configuration → Companion Devices page and press the width you want. Leaving 1-byte asks you to confirm first, because older repeaters cannot forward wider hashes; a line under the keys shows the command being sent and the companion accepting it, and the next message you send prints the new width on its row.
- **Companion Devices page.** A home in the Configuration workspace for the settings that live on each companion itself, shown as an instrument in the same style as Regions Management. Pick a companion, connect, and set.

### Changed
- **The repeater's own Path Hash Size field now says whose radio it is.** It sets the repeater's width only; each companion has its own, and the two never affect each other. Dropdown settings across Configuration can now carry a help note and an info tooltip.

### Fixed
- **Moving between Messages, Regions Management and Companion Devices no longer drops the companion link.** The link was torn down and reopened on every move, and the repeater briefly reported the console's own previous session as "another client". The link now stays up across all companion pages.
- **Manual paths at 2- and 3-byte widths.** The contact path editor accepted only 1-byte hops and could mis-encode a path on a companion set to a wider hash. Each hop can now be 2, 4 or 6 hex characters, all the same width, and the path is sent at the width you typed.
- **Bot path and test reports on wider hashes.** HOWL bot replies with 2- or 3-byte route hashes were read as the wrong hops; they now parse whole.

## [0.9.393] - 2026-09-14
### Fixed
- refine regions hardware and screen hierarchy

## [0.9.392] - 2026-09-14
### Added
- **Regions have their own management page.** Review nearby repeaters' saved region reports, add regions, and manage repeater forwarding rules and companion scope settings from one place. Discovery requires an explicit repeater selection before Scan is enabled; querying all nearby repeaters is an opt-in choice with an airtime warning.

### Changed
- **Region controls fit the available panel width.** Saved-report counts share one row, default-scope actions sit beside their selector when space allows, and companion scope summaries sit side by side on wider panels. The discovery instrument uses a compact hardware face and ribbon connection to the settings below.
- **Companion selectors preserve device names.** Names keep their original case and characters rather than being shortened into uppercase hardware labels.
- **Message region labels distinguish intent from delivery.** Labels describe the requested region or locally known settings, without claiming that radio coverage or the packet's actual scope was verified.

### Fixed
- **Failed browser-storage resets remain visible.** If saved channel-region settings cannot be cleared, the console reports the failure instead of pretending the reset succeeded.

---

## [0.9.391] - 2026-09-14
### Added
- **Per-channel region overrides work now.** Sending one message to a named region, without changing the region your node is otherwise on, needed matching support in openHop Core. That has shipped, so a repeater running the current Core can do it and the console will use it. The console checks each connection and says so plainly when a repeater is too old, rather than sending the message somewhere you did not name.
- **A message sent to a region says so plainly.** A message you sent under a per-channel region override now carries a filled tag naming that region, the same solid lit cell the unread counts use, instead of a line of dim text beside the delivery marks. It went somewhere other than your node's usual region, and that is the one thing about it worth not missing.

### Fixed
- **The message counter knows a region override costs room.** A message sent to a named region carries that region's key, which leaves seven fewer bytes for your text than an ordinary channel message. If your node's name was four characters or shorter, the composer would let you write past what such a message can carry and the send would then fail. The counter now stops the draft at the real wall.
- **A channel region keeps the capitals you typed.** Typing `#USA` as a per-channel region used to save and send it as `#usa`. Those are two different regions on the mesh, and they reach different people, so the console was quietly moving your messages somewhere you had not asked for. Regions now keep their case exactly as entered, and `#USA` and `#usa` can both sit in your saved list as the separate regions they are. Regions you saved before this are unchanged, in the lowercase form the list has been showing you; add the capitalised name as a new entry if that is the one you meant. A region name is still letters, digits and hyphens only, and a lookalike letter from another alphabet is still refused.

### Changed
- **A region now travels as its key rather than its name.** The agreed format for this changed while the feature was being built on both sides, and the message now carries the region's key. That is what makes the capitals above matter. The console asks each repeater which version of the format it speaks and refuses to send an override to one that answers anything else, so a mismatch leaves the feature unavailable rather than delivering to the wrong region.
